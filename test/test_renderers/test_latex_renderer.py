...
class TestLaTeXRenderer(TestCase):
    ...
    def _test_token(
        self, token_name, output, children=True, without_attrs=None, **kwargs
    ):
    ...
    def test_math(self):
        output = "$ 1 + 2 = 3 $"
        self._test_token("Math", output, children=False, content="$ 1 + 2 = 3 $")

    def test_raw_text(self):
        output = "\\$\\&\\#\\{\\}"
        self._test_token("RawText", output, children=False, content="$&#{}")
    ...
    def test_table_without_header(self):
        ...
        self._test_token("Table", output, without_attrs=["header"], column_align=[None])
